...
// Создаем двумерную карту для хранения лучей
const BEAM_AXE = new Map();
// Функция для инициализации карты
function initBeamAxeMap() {
    // Проходим по всем возможным парам p и v
    for (let p = 0; p <= MAX_FIELD_Axe; p++) {
        for (let v = 0; v <= MAX_FIELD_Axe; v++) {
            // Получаем луч через текущую функцию
            const beam = getBeamAxe(p, v);
            
            // Создаем ключ для карты
            const key = `${p},${v}`;
            
            // Сохраняем типизированный массив в карту
            BEAM_AXE.set(key, beam);
        }
    }
}
// Функция для получения луча из карты
function mapBeamAxe(p, v) {
    // Получаем ключ
    const key = `${p},${v}`;
    
    // Возвращаем сохраненный типизированный массив
    return BEAM_AXE.get(key) || new Uint8Array();
}
...
// Создаем двумерную карту для хранения лучей пиков
const BEAM_PIKE = new Map();
// Функция для инициализации карты
function initBeamPikeMap() {
    // Проходим по всем возможным парам p и v
    for (let p = 0; p <= MAX_FIELD_Pike; p++) {
        for (let v = 0; v <= MAX_FIELD_Pike; v++) {
            ...
            // Сохраняем типизированный массив в карту
            BEAM_PIKE.set(key, beam);
        }
    }
}
// Функция для получения луча из карты
function mapBeamPike(p, v) {
    // Получаем ключ
    const key = `${p},${v}`;
    
    // Возвращаем сохраненный типизированный массив
    return BEAM_PIKE.get(key) || new Uint8Array();
}
// Инициализируем карту при загрузке
initBeamPikeMap();
...
// Создаем карту для хранения лучей стрелков
const BEAM_ARROW = new Map();
// Функция для инициализации карты
function initBeamArrowMap() {
    // Проходим по всем возможным парам p и v
    for (let p = 0; p <= MAX_FIELD_Arrow; p++) {
        for (let v = 0; v <= MAX_FIELD_Arrow; v++) {
            // Получаем луч через текущую функцию
            const beam = getBeamArrow(p, v);
            
            // Создаем ключ для карты
            const key = `${p},${v}`;
            
            // Сохраняем типизированный массив в карту
            BEAM_ARROW.set(key, beam);
        }
    }
}
// Функция для получения луча из карты
function mapBeamArrow(p, v) {
    // Получаем ключ
    const key = `${p},${v}`;
    
    // Возвращаем сохраненный типизированный массив
    return BEAM_ARROW.get(key) || new Uint8Array();
}
// Инициализируем карту при загрузке
initBeamArrowMap();
...
// countArrowMove = 0; countArrowCapture = 0;
for (let p = 0; p <= MAX_FIELD_Arrow; p++) {
    for (let v = 0; v <= MAX_FIELD_Arrow; v++) {   
...
//console.log(countArrowMove, countArrowCapture);
...
const  
    ...
    //Бонусы мобильности по зонам (для Ωfull)
    MOBILITY_BONUS = {
    ...
    // Позиции в system (offset-ы)
    FIELD_UNIT = 0,      // 93 элемента: 0–92
    UNIT_FIELD = 93,     // 53 элемента: 93–145
    UNIT_PREV = 146,     // 53 элемента: 146–198
    UNIT_SIDE = 199,     // 53 элемента: 199–251 (0=null, 1=true, 2=false)
    UNIT_SORT = 252,     // 53 элемента: 252–304
    UNIT_CASTLING = 305, // 53 элемента: 305–357
    QUEUE = 358,         // 1 элемент
    MOVE = 359;          // 1 элемент
...
//////////////////////////////////////////////////////////////////////////////////////////////
/////// оформление и визуализации в Консоли //////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////
function alignSort(sort) {
    const widths = {
        [helm]:   "Король№",
        [sword]:  "─Ферзь№",
        [axe]:    "─Ладья№",
...
const PIKE_RAY = new Map();  // все лучи слонов
const AXE_RAY = new Map();   // все лучи ладей

//////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////// = Предрасчёты = //////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////
// для всех дистанций по сетке клеток
...
// для определения луч или не луч
...
// для коней, прыжки все возможные
...
// для getExchange()
function precomputePikeRayMap() {
    for (let v = 1; v <= MAX_FIELD_Pike; v++) {
        if (!starPike[v]) continue;
        
        for (let pathIdx = 1; pathIdx < starPike[v].length; pathIdx++) {
            const path = starPike[v][pathIdx];
            
            if (!Array.isArray(path) || path.length < 2) continue;
            
            // Заменяем 0 на центральный индекс v
            const modifiedPath = [v, ...path.slice(1)];
            
            // Формируем ключ по первой и второй клетке пути
            const start = modifiedPath[0];
            const next = modifiedPath[1]; // берём соседнюю клетку
            
            const key = `${start},${next}`;
            
            const rayPath = new Uint8Array(modifiedPath);
            
            PIKE_RAY.set(key, rayPath);
        }
    }
}
function precomputeAxeRayMap() {
    for (let v = 1; v <= MAX_FIELD_Axe; v++) {
        if (!starAxe[v]) continue;
...
// для получения слоновьего луча по началу и концу
function getPikeRay(f0, f1) {
    const key = `${f0},${f1}`;
    return PIKE_RAY.get(key);
}
// для получения ладейного луча по началу и концу
function getAxeRay(f0, f1) {
    const key = `${f0},${f1}`;
    return AXE_RAY.get(key);
}
// для коней
function jumps(u, v) {
...
function getRayOfFields(u, v) {
    if (u === v) return null; // Пропускаем одинаковые клетки
    
    // Сначала проверяем PIKE_RAY
    for (let [key, path] of PIKE_RAY) {
        const [start, next] = key.split(',').map(Number);
        
        // Проверяем соответствие начальной клетки и наличие v в пути
        if (start === u && path.includes(v)) {
            return path; // Возвращаем только путь
        }
    }
    
    // Затем проверяем AXE_RAY
    for (let [key, path] of AXE_RAY) {
        const [start, next] = key.split(',').map(Number);
    ...
    return null; // Если луч не найден
}

function isCellAvailable(u, v, exchangeSystem) {
    // Получаем луч между клетками
    const ray = getRayOfFields(u, v);
    if (!ray) return false;

    // Находим позиции клеток в луче
    const uIndex = ray.indexOf(u);
    const vIndex = ray.indexOf(v);

    // Базовая проверка порядка клеток
    if (vIndex <= uIndex) return false;

    // Проверяем все клетки между u и v
    for (let i = uIndex + 1; i < vIndex; i++) {
        const cell = ray[i];
        // Проверяем, занята ли клетка фигурой
        if (exchangeSystem[FIELD_UNIT + cell] !== 0) {
            return false;
        }
    }
    return true;
...
for (let u = 1; u <= 92; u++) {
    starExchange[u] = new Uint8Array(93); // максимальный размер массива
    starExchange[u].fill(0); // инициализируем нулями
}

// Заполнение starExchange[u]
for (let u = 1; u <= 92; u++) {
    let index = 1; // первый элемент будет содержать саму клетку u
    starExchange[u][0] = u;

    // Функция проверки наличия элемента
    function isExists(cell) {
        for (let i = 0; i < index; i++) {
            if (starExchange[u][i] === cell) return true;
        }
        return false;
    }
    ...
    for (const cell of axeSet) {
        if (cell !== 0 && !isExists(cell)) {
            starExchange[u][index] = cell;
            index++;
        }
    }
    ...
    starExchange[u] = starExchange[u].slice(0, index);
}



function makeExchangeSystem(currentSystem, u) {
    const FIELD_UNIT = 0; // 93 элемента: 0–92
    const UNIT_FIELD = 93; // 53 элемента: 93–145
    const UNIT_PREV  = 146; // 53 элемента: 146–198
    const UNIT_SIDE  = 199; // 53 элемента: 199–251
    const UNIT_SORT  = 252; // 53 элемента: 252–304
    const INIT_SIDE  = 305; // 1 элемент
    
    const NOT_IN_EXCHANGE = 0xFF; // Маркер для клеток вне starExchange
    
    const exchangeSystem = new Uint8Array(306);
    
    const targetUnitId = currentSystem[FIELD_UNIT + u];
    const initSide = 3 - currentSystem[UNIT_SIDE + targetUnitId];
    
    // Инициализируем базовую информацию
    exchangeSystem[INIT_SIDE] = initSide;
    exchangeSystem[FIELD_UNIT + u] = targetUnitId;
    
    // Заполняем все клетки значением NOT_IN_EXCHANGE
    for(let i = 0; i < 93; i++) {
        exchangeSystem[FIELD_UNIT + i] = NOT_IN_EXCHANGE;
    }
    
    // Копируем информацию только о клетках в starExchange
    for (let cell of starExchange[u]) {
        const unitId = currentSystem[FIELD_UNIT + cell];
        
        exchangeSystem[FIELD_UNIT + cell] = unitId;
        exchangeSystem[UNIT_FIELD + unitId] = cell;
        exchangeSystem[UNIT_SIDE + unitId] = currentSystem[UNIT_SIDE + unitId];
        exchangeSystem[UNIT_SORT + unitId] = currentSystem[UNIT_SORT + unitId];
        exchangeSystem[UNIT_PREV + unitId] = currentSystem[UNIT_PREV + unitId];
    }
